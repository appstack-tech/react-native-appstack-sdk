...
- **Breaking:** `sendEvent(eventType, eventName, parameters)` is now `sendEvent(event, parameters)`. The middle `eventName` argument is gone. Pass a standard `EventType` for a standard event, or your own name for a custom one — there is no separate "custom" mode:
  ...
  ```
  A three-argument call now throws with a message naming the replacement, rather than silently binding the event name to `parameters` and shipping an event whose payload is its own name. `EventType` itself stays, and remains the recommended form; a plain string is accepted and resolved case-insensitively.
...
- **`INSTALL`, `FIRST_OPEN` and `FIRST_OPEN_GUARDED` are dropped before reaching native**, logging an error and resolving normally rather than throwing. These are recorded automatically and sending them by hand inflates install counts, which is why iOS already discarded them natively. `FIRST_OPEN` and `FIRST_OPEN_GUARDED` exist only in iOS's enum, so a manual send previously became a bogus *custom* event named `FIRST_OPEN` on Android alone. Resolving rather than rejecting keeps callers who harmlessly send a manual `INSTALL` today — a silent discard since 2.5.0 — from suddenly seeing failures.
